...
//Tool to read and (optionally) modify serial number and/or date from PIC16x57 dump used in many Atari/Midway games
//by Pat Daderko (DogP) based on MAME midway_serial_pic_device::generate_serial_data from midwayic.cpp by Aaron Giles
//
//Usage: midway_pic_tool <filename> [new SN (0-999999)] [new MM (1-12)] [new DD (0-30)] [new YYYY (1980-2155)]
...
int main(int argc, char* argv[])
{
    ...
    uint32_t serial_number, serial_gameid;
    ...
    serial_gameid = serial_digit[0] * 100 + serial_digit[1] * 10 + serial_digit[2];
    serial_number = serial_digit[3] * 100000 + serial_digit[4] * 10000 + serial_digit[5] * 1000 + serial_digit[6] * 100 + serial_digit[7] * 10 + serial_digit[8];

    printf("Original S/N: %03d%06d\n", serial_gameid, serial_number);
    ...
    {
        temp = atoi(argv[2]);
        if (strcmp(argv[2], "random") == 0) { //if random serial is requested
            printf("Random serial requested\n");
            temp = (seed % (999999 - 100000 + 1)) + 100000;
        }
